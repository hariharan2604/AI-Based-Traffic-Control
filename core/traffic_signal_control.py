...
def handle_emergency(mqtt_client, emergency_pair):
    ...
    logging.info(f"🟡 Turning {active_signal} yellow for {BASE_YELLOW_DURATION}s")
    for signal in active_signal:
    ...
    logging.info(f"🔴 Turning {active_signal} red for {red_duration}s")
    for signal in active_signal:
        update_signal(mqtt_client, signal, "red", red_duration)

    green_duration = ACO_MAX_DURATION + EMERGENCY_GREEN_BOOST
    logging.info(f"🟢 Turning {emergency_pair} green for {green_duration}s")
    ...
    for _ in range(int(ACO_MAX_DURATION / 2)):
        ...
        if new_emergency and new_emergency != emergency_pair:
            logging.info(
                f"🚨 New emergency detected: {new_emergency}! Switching immediately."
            )
...
def cycle_signals(mqtt_client, ws_servers):
    ...
    while True:
        while not all(ws.client_count > 0 for ws in ws_servers):
            logging.info("⏸️ Waiting for WebSocket clients...")
            time.sleep(2)

            if time.time() - start_time > timeout:
                logging.info(
                    "⚠️ Timeout waiting for WebSocket clients. Proceeding with default settings."
                )
                break
        ...
        if not initialized:
            logging.info("🔄 Reinitializing signals...")
        ...
        logging.info(f"📢 Current Emergency Events: {emergency_events}")

        emergency_pair = check_emergency_interrupt()
        logging.info(f"📢 Current Emergency pair: {emergency_pair}")
        if emergency_pair:
        ...
        if not density_data:
            logging.info("⚠️ No density data available. Using default cycle.")
            time.sleep(2)
            continue
        ...
        logging.info(
            f"🟡 Transitioning {current_pair} to yellow for {BASE_YELLOW_DURATION}s"
        )
        ...
        logging.info(f"🟢 Setting {next_pair} to green for {green_duration}s")
        for signal in next_pair:
            update_signal(mqtt_client, signal, "green", green_duration)

        red_duration = green_duration + BASE_YELLOW_DURATION
        logging.info(f"🔴 Setting {current_pair} to red for {red_duration}s")
        for signal in current_pair:
            update_signal(mqtt_client, signal, "red", red_duration)
