...
class WebSocketServer:
    ...
    def new_client(self, client, server):
        ...
        logging.info(f"✅ [WebSocket {self.port}] New client connected: {client} (Total: {self.client_count})")

    def client_left(self, client, server):
        self.client_count -= 1
        if self.client_count == 0:
            self.client_event.clear()
        logging.info(f"❌ [WebSocket {self.port}] Client disconnected: {client} (Remaining: {self.client_count})")
    ...
    def close(self):
        logging.info(f"🛑 [WebSocket {self.port}] Server shutting down.")
        try:
            self.server.shutdown()
        except Exception as e:
            logging.error(f"⚠️ Error closing WebSocket server: {e}")
        self.client_event.clear()
